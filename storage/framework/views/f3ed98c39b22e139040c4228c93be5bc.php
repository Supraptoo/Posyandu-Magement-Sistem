    ...
    <aside id="sidebar" class="fixed top-0 left-0 h-screen w-[280px] glass-sidebar border-r border-slate-200/80 z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-out flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.03)] overflow-hidden">
        ...
        <div class="p-5 pb-2 shrink-0 relative z-10">
            <div class="p-3.5 bg-white border border-slate-200 rounded-[20px] flex items-center gap-3 hover:border-indigo-300 hover:shadow-sm transition-all cursor-pointer group" onclick="document.getElementById('userDropdownBtn').click()">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-[13px] font-bold text-slate-800 truncate font-poppins leading-tight"><?php echo e(Auth::user()->profile->full_name ?? Auth::user()->name ?? 'Kader Posyandu'); ?></p>
        ...
        <header class="h-20 glass-header border-b border-white/50 sticky top-0 z-40 flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
            <div class="flex items-center gap-4">
                ...
                <nav class="hidden sm:flex items-center gap-2 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    ...
                    <span class="text-indigo-600"><?php echo $__env->yieldContent('page-name', 'Dashboard'); ?></span>
                </nav>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-4 relative">
                ...
                <div class="static sm:relative">
                    <button id="notifDropdownBtn" class="relative w-10 h-10 flex items-center justify-center bg-white text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-all border border-slate-200 shadow-sm hover:shadow">
                        ...
                        <span id="notifBadge" class="absolute top-0 right-0 w-3 h-3 bg-rose-500 rounded-full ring-2 ring-white animate-pulse <?php echo e($unreadNotifCount > 0 ? '' : 'hidden'); ?>"></span>
                    </button>
                        ...
                        <div id="notifList" class="max-h-72 overflow-y-auto custom-scrollbar flex-1">
                            <?php $__empty_1 = true; $__currentLoopData = $latestNotifs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <a href="<?php echo e(route('kader.notifikasi.index')); ?>" class="notif-item <?php echo e($n->is_read ? '' : 'unread'); ?> flex gap-4 px-5 py-4 hover:bg-slate-50 transition-colors border-b border-slate-100 <?php echo e($n->is_read ? 'bg-white border-l-4 border-l-transparent' : 'bg-indigo-50/40 border-l-4 border-l-indigo-500'); ?>">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border <?php echo e($n->is_read ? 'bg-slate-50 text-slate-400 border-slate-200' : 'bg-indigo-100 text-indigo-600 border-indigo-200'); ?>">
                                    ...
                                    <div>
                                        <p class="text-[13px] font-bold <?php echo e($n->is_read ? 'text-slate-600' : 'text-slate-800'); ?> leading-tight font-poppins"><?php echo e($n->judul); ?></p>
                                        <p class="text-[11px] text-slate-500 mt-1 line-clamp-2 leading-relaxed"><?php echo e($n->pesan); ?></p>
                                    </div>
                                </a>
                ...
                <div class="static sm:relative ml-1">
                    ...
                    <div id="userDropdown" class="hidden absolute top-16 right-0 w-[calc(100vw-2rem)] sm:top-auto sm:right-0 sm:mt-3 sm:w-64 bg-white rounded-[24px] shadow-[0_15px_50px_-10px_rgba(0,0,0,0.1)] border border-slate-100 z-50 animate-pop overflow-hidden">
                        <div class="px-5 py-5 border-b border-slate-50 bg-gradient-to-r from-indigo-50 to-violet-50">
                            <p class="text-[15px] font-black text-slate-800 truncate font-poppins"><?php echo e(Auth::user()->profile->full_name ?? Auth::user()->name ?? 'Kader'); ?></p>
                            <p class="text-[10px] text-indigo-600 font-bold uppercase tracking-widest mt-1"><?php echo e(Auth::user()->email ?? 'kader@posyandu.com'); ?></p>
                        </div>
                        <div class="p-2 border-t border-slate-50">
                            <a href="<?php echo e(route('kader.profile.index')); ?>" class="w-full flex items-center gap-3 px-4 py-3 text-[13px] font-bold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all"><i class="fas fa-user-circle w-4 text-center"></i> Profil Saya</a>
            ...
            <?php $__currentLoopData = ['success' => ['bg-emerald-50', 'text-emerald-800', 'text-emerald-500', 'fa-check-circle', 'border-emerald-200'], 'error' => ['bg-rose-50', 'text-rose-800', 'text-rose-500', 'fa-circle-exclamation', 'border-rose-200']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg => $classes): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(session($msg)): ?>
                    <div class="mb-6 px-5 py-4 <?php echo e($classes[0]); ?> border <?php echo e($classes[4]); ?> rounded-2xl flex items-center justify-between shadow-sm animate-[slideDown_0.4s_ease-out]">
                        <div class="flex items-center gap-3"><i class="fas <?php echo e($classes[3]); ?> <?php echo e($classes[2]); ?> text-xl"></i><span class="<?php echo e($classes[1]); ?> text-[13px] sm:text-sm font-bold"><?php echo e(session($msg)); ?></span></div>
