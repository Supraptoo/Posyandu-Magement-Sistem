...
<body class="text-slate-800 antialiased lg:pb-0 pb-[80px]"> <div id="userLoader" class="fixed inset-0 bg-slate-50/90 backdrop-blur-md z-[9999] flex flex-col items-center justify-center transition-all duration-400 opacity-0 pointer-events-none">
        ...
        <div class="flex-1 lg:ml-[280px] flex flex-col w-full min-w-0 transition-all duration-300">
            ...
            <header class="h-16 glass-effect border-b border-slate-200/80 sticky top-0 z-30 flex items-center justify-between px-5 sm:px-8 shadow-sm">
                ...
                <div class="flex items-center gap-3 sm:gap-4">
                    <a href="<?php echo e(route('user.notifikasi.index')); ?>" class="smooth-route relative w-10 h-10 flex items-center justify-center text-slate-500 hover:text-teal-600 hover:bg-teal-50 rounded-full transition-all bg-white border border-slate-100 shadow-sm">
                    ...
                    <a href="<?php echo e(route('user.profile.edit')); ?>" class="smooth-route w-10 h-10 rounded-full ring-2 ring-slate-100 overflow-hidden flex items-center justify-center bg-gradient-to-br from-teal-50 to-sky-50 text-teal-700 font-extrabold shadow-sm hover:ring-teal-300 transition-all border border-teal-200">
            ...
            <main class="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-6xl mx-auto relative z-0">
                <?php $__currentLoopData = ['success' => ['bg-emerald-50', 'text-emerald-600', 'fa-check-circle', 'border-emerald-200'], 'error' => ['bg-rose-50', 'text-rose-600', 'fa-exclamation-circle', 'border-rose-200']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg => $cls): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if(session($msg)): ?>
                        <div class="mb-5 px-4 py-3.5 <?php echo e($cls[0]); ?> border <?php echo e($cls[3]); ?> rounded-2xl flex items-center justify-between shadow-sm animate-[slideDown_0.4s_ease-out]">
                            <div class="flex items-center gap-3">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 w-full glass-effect border-t border-slate-200/80 z-50 pb-safe rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <div class="flex justify-between items-center px-4 pt-1">
            ...
            <div class="relative -top-6 px-3">
                <a href="<?php echo e(route('user.riwayat.index')); ?>" class="smooth-route w-14 h-14 rounded-full bg-gradient-to-tr from-teal-500 to-sky-500 text-white flex items-center justify-center text-2xl shadow-[0_8px_20px_rgba(13,148,136,0.4)] border-[4px] border-[#f8fafc] hover:scale-105 transition-transform btn-pulse">
    ...
    <script>
        ...
        function checkNotifications() {
            fetch('<?php echo e(route("user.notifikasi.latest")); ?>', {
