...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'SIPOSYANDU'); ?> — SIPOSYANDU</title>
...
<nav class="sp-nav">
    ...
    <a href="<?php echo e(route('home')); ?>" class="sp-brand">
    ...
    <div class="sp-right">
        ...
        <a href="<?php echo e(route('user.notifikasi.index')); ?>" class="sp-bell" title="Notifikasi">
        ...
        <div class="sp-drop">
            <div class="sp-user" role="button" tabindex="0">
                <div class="sp-av">
                    <?php echo e(strtoupper(substr(auth()->user()->profile?->full_name ?? auth()->user()->name ?? 'U', 0, 1))); ?>

                </div>
                <span class="sp-uname">
                    <?php echo e(Str::limit(auth()->user()->profile?->full_name ?? auth()->user()->name, 18)); ?>

                </span>
                <span class="sp-role-chip"><?php echo e(auth()->user()->role); ?></span>
...
<aside class="sp-sidebar" id="spSidebar" role="navigation">
    ...
    <?php elseif(auth()->user()->role === 'bidan'): ?>
...
// ── Flash Messages ──
<?php if(session('success') && !session('generated_password') && !session('reset_password')): ?>
    _T.fire({ icon: 'success', title: '<?php echo e(addslashes(session("success"))); ?>' });
<?php endif; ?>
<?php if(session('error')): ?>
    _T.fire({ icon: 'error', title: '<?php echo e(addslashes(session("error"))); ?>' });
<?php endif; ?>
<?php if(session('warning')): ?>
    _T.fire({ icon: 'warning', title: '<?php echo e(addslashes(session("warning"))); ?>' });
<?php endif; ?>
<?php if(session('info')): ?>
    _T.fire({ icon: 'info', title: '<?php echo e(addslashes(session("info"))); ?>' });
<?php endif; ?>
