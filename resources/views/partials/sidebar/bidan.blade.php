...
<div class="space-y-7">
    ...
    <div>
        ...
        <div class="space-y-2">
            <button onclick="toggleSubmenu('menuPasien', 'iconPasien')" class="w-full group flex items-center justify-between px-4 py-3 rounded-2xl transition-all duration-300 font-bold text-sm border border-transparent {{ $isDataWargaActive ? 'bg-cyan-50/80 text-cyan-700 border-cyan-100' : 'text-slate-500 hover:bg-slate-50 hover:text-cyan-700 hover:border-slate-100' }}">
                <div class="flex items-center gap-3">
                    <i class="fas fa-users-viewfinder w-6 text-center text-[18px] transition-colors {{ $isDataWargaActive ? 'text-cyan-600' : 'text-slate-400 group-hover:text-cyan-500' }}"></i>
                    <span class="font-poppins tracking-wide">Database Warga</span>
                </div>
                <i id="iconPasien" class="fas fa-chevron-down text-[10px] transition-transform duration-300 {{ $isDataWargaActive ? 'rotate-180 text-cyan-600' : 'text-slate-400' }}"></i>
            </button>
