    ...
    <div class="card-body p-0">
        <div class="bg-primary p-4 text-white position-relative overflow-hidden" style="min-height: 150px;">
            ...
            <div class="d-flex align-items-center position-relative z-index-1">
                ...
                <div>
                    <h2 class="fw-bold mb-1">{{ $balita->nama_lengkap }}</h2>
                    <div class="d-flex gap-2 mb-2">
                        <span class="badge bg-white text-primary"><i class="fas fa-id-card me-1"></i> {{ $balita->nik }}</span>
                        <span class="badge bg-{{ $balita->jenis_kelamin == 'L' ? 'info' : 'danger' }} text-white">
                            <i class="fas fa-venus-mars me-1"></i> {{ $balita->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}
                        </span>
                    </div>
                </div>
                <div class="ms-auto d-none d-md-block text-end">
                    <a href="{{ route('kader.data.balita.edit', $balita->id) }}" class="btn btn-light text-primary fw-bold rounded-pill px-4 shadow-sm">
                ...
                <div class="row g-3 text-center">
                    <div class="col-6">
                        <div class="p-3 bg-light rounded-3">
                            ...
                            <div class="h5 fw-bold text-dark mb-0">{{ $balita->berat_lahir ?? '-' }} <small>kg</small></div>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="p-3 bg-light rounded-3">
                            <div class="small text-muted mb-1">Panjang Lahir</div>
                            <div class="h5 fw-bold text-dark mb-0">{{ $balita->panjang_lahir ?? '-' }} <small>cm</small></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div>
                        ...
                        <div class="fw-bold text-dark">{{ $balita->nama_ibu }}</div>
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div>
                        ...
                        <div class="fw-bold text-dark">{{ $balita->nama_ayah ?? '-' }}</div>
                    </div>
                </div>
                ...
                <div class="d-flex mb-3">
                    ...
                    <div>
                        ...
                        <div class="text-dark">{{ $balita->alamat }}</div>
                    </div>
                </div>
                ...
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        <div class="text-dark">{{ $balita->tempat_lahir }}, {{ $balita->tanggal_lahir->format('d M Y') }}</div>
                        <small class="text-primary fw-bold">
                            {{ $usia_tahun > 0 ? $usia_tahun . ' Th ' : '' }}{{ $sisa_bulan }} Bulan
                        </small>
                    </div>
                </div>
            </div>
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-header bg-white border-bottom-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('kader.pemeriksaan.create') }}?pasien_id={{ $balita->id }}&pasien_type=balita" class="btn btn-primary btn-sm rounded-pill px-3 shadow-sm">
                    <i class="fas fa-plus me-1"></i> Kunjungan Baru
                </a>
            </div>
            <div class="card-body p-4">
                @if($balita->kunjungans && $balita->kunjungans->count() > 0)
                    <div class="timeline">
                        @foreach($balita->kunjungans as $kunjungan)
