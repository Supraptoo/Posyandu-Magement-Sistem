...
<div class="space-y-6 sm:space-y-8 pb-4">
    ...
    <div class="animate-slide-up bg-white rounded-[24px] p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h2 class="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight leading-tight">Halo, {{ explode(' ', Auth::user()->name)[0] }}! 👋</h2>
    ...
    <div class="animate-slide-up-delay bg-gradient-to-r from-rose-50 to-orange-50 border border-rose-200 p-5 rounded-[20px] flex items-start sm:items-center gap-4 shadow-sm">
        ...
        <div class="flex-1">
            ...
            <p class="text-xs font-medium text-rose-600 mt-1 mb-2.5 leading-snug">{{ $pesanError }}</p>
            <a href="{{ route('user.profile.edit') }}" class="smooth-route inline-flex items-center gap-1.5 px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl shadow-sm transition-all hover:-translate-y-0.5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div class="animate-slide-up-delay">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('user.notifikasi.index') }}" class="smooth-route text-[11px] font-bold text-teal-600 hover:text-teal-800 bg-teal-50 px-3 py-1.5 rounded-lg">Semua Info</a>
            </div>
            
            <div class="bg-white border border-slate-100 rounded-[24px] shadow-sm overflow-hidden p-2">
                @forelse($notifikasiTerbaru ?? [] as $notif)
                    <a href="{{ route('user.notifikasi.index') }}" class="flex items-start gap-4 p-4 hover:bg-slate-50 transition-colors rounded-xl {{ !$notif['is_read'] ? 'bg-sky-50/50' : '' }}">
                        <div class="w-10 h-10 rounded-full {{ !$notif['is_read'] ? 'bg-sky-100 text-sky-600' : 'bg-slate-100 text-slate-400' }} flex items-center justify-center text-lg shrink-0">
                            <i class="fas fa-envelope-open-text"></i>
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start gap-2 mb-1">
                                <h4 class="text-sm font-bold text-slate-800 truncate">{{ $notif['judul'] }}</h4>
                                @if(!$notif['is_read']) <span class="w-2 h-2 rounded-full bg-rose-500 shrink-0 mt-1.5"></span> @endif
                            </div>
                            <p class="text-xs text-slate-500 line-clamp-2 leading-relaxed">{{ $notif['pesan'] }}</p>
                            <span class="text-[10px] font-bold text-slate-400 mt-2 block"><i class="fas fa-clock mr-1"></i> {{ $notif['waktu'] }}</span>
                        </div>
                    </a>
                @empty
        ...
        <div class="animate-slide-up-delay-2 space-y-6 sm:space-y-8">
            ...
            <div>
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('user.jadwal.index') }}" class="smooth-route text-[11px] font-bold text-sky-600 hover:text-sky-800 bg-sky-50 px-3 py-1.5 rounded-lg">Cek Kalender</a>
                </div>
                ...
                <div class="bg-gradient-to-r from-teal-600 to-sky-600 rounded-[24px] p-6 text-white shadow-[0_12px_30px_rgba(13,148,136,0.3)] relative overflow-hidden group mb-4 last:mb-0 transform transition-transform hover:-translate-y-1">
                    ...
                    <div class="relative z-10">
                        <span class="inline-block px-2.5 py-1 bg-white/20 backdrop-blur-md rounded-lg text-[10px] font-black tracking-widest uppercase mb-3 border border-white/20 shadow-sm">{{ $jadwal->kategori }}</span>
                        <h4 class="font-black text-xl leading-tight mb-4 tracking-tight pr-10">{{ $jadwal->judul }}</h4>
                        ...
                        <div class="bg-black/10 backdrop-blur-sm rounded-xl p-3.5 border border-white/10 flex flex-col gap-2">
                            <div class="flex items-center gap-3 text-xs font-semibold text-teal-50">
                                ...
                                <span>{{ \Carbon\Carbon::parse($jadwal->tanggal)->translatedFormat('d F Y') }} • {{ date('H:i', strtotime($jadwal->waktu_mulai)) }} WIB</span>
                            </div>
                            <div class="flex items-center gap-3 text-xs font-semibold text-teal-50">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <a href="{{ route('user.balita.index') }}" class="smooth-route bg-white border border-slate-100 p-5 rounded-[20px] shadow-sm hover:shadow-[0_8px_20px_rgba(244,63,94,0.15)] hover:border-rose-200 transition-all flex flex-col justify-center items-center text-center group">
                        ...
                        <p class="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-wider">{{ isset($dataAnak) ? $dataAnak->count() : 0 }} Terdaftar</p>
                    </a>

                    <a href="{{ route('user.remaja.index') }}" class="smooth-route bg-white border border-slate-100 p-5 rounded-[20px] shadow-sm hover:shadow-[0_8px_20px_rgba(14,165,233,0.15)] hover:border-sky-200 transition-all flex flex-col justify-center items-center text-center group">
                    ...
                    <a href="{{ route('user.lansia.index') }}" class="smooth-route bg-white border border-slate-100 p-5 rounded-[20px] shadow-sm hover:shadow-[0_8px_20px_rgba(245,158,11,0.15)] hover:border-amber-200 transition-all flex flex-col justify-center items-center text-center group col-span-2">
